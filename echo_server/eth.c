...
uintptr_t eth_hw_ring_buffer_vaddr;
uintptr_t eth_hw_ring_buffer_paddr;
uintptr_t eth_dma_vaddr;
uintptr_t eth_dma_paddr;
uintptr_t eth_rx_cookies;
uintptr_t eth_tx_cookies;
uintptr_t eth_rx_free;
uintptr_t eth_rx_used;
...
{
    uint64_t offset = virtual - eth_dma_vaddr;
    uintptr_t phys;
...
eth_setup(void)
{
    ...
    rx.tail = 0;
    rx.head = 0;
    rx.phys = eth_dma_paddr;
    rx.cookies = (void **)eth_rx_cookies;
    rx.descr = (volatile struct descriptor *)eth_hw_ring_buffer_vaddr;

    tx.cnt = TX_COUNT;
    tx.remain = tx.cnt - 2;
    tx.tail = 0;
    tx.head = 0;
    tx.phys = eth_dma_paddr + (sizeof(struct descriptor) * RX_COUNT);
    tx.cookies = (void **)eth_tx_cookies;
    ...
    /* Set RDSR */
    eth->rdsr = eth_hw_ring_buffer_paddr;
    eth->tdsr = eth_hw_ring_buffer_paddr + (sizeof(struct descriptor) * RX_COUNT);
